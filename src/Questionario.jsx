...
export default function Questionario() {
  ...
  const enviar = () => {
    ...
    setTimeout(() => { setSalvando(false); setEnviado(true); window.scrollTo({ top: 0, behavior: 'smooth' }); }, 700);
  };
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={st.container}>
        ...
        <div style={{ ...st.card, borderTop: '4px solid #2E75B6' }}>
          <h2 style={st.secH2(false)}>Identificação</h2>
          <div style={st.grid2}>
          ...
          <div key={bloco.key} style={st.card}>
            ...
            <div style={{ border: '1px solid #E8E8E8', borderRadius: 10, overflow: 'hidden', marginTop: 14 }}>
              {bloco.indicadores.map((ind, idx) => {
                ...
                return (
                  <div key={idx} style={{ borderBottom: idx < bloco.indicadores.length - 1 ? '1px solid #E8E8E8' : 'none' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', padding: '14px 16px', gap: 14, background: marcado ? '#EAF3FB' : par ? '#F5F5F5' : '#fff' }}>
                      ...
                      <div style={{ display: 'flex', gap: 7, flexShrink: 0 }}>
                        {CONCEITOS.map(c => (
                          <button
                            key={c.valor}
                            title={c.label}
                            onClick={() => setResposta(bloco.key, idx, c.valor)}
                            style={{ width: 36, height: 36, borderRadius: 7, border: marcado === c.valor ? 'none' : `2px solid ${c.cor}55`, background: marcado === c.valor ? c.cor : 'transparent', color: marcado === c.valor ? '#fff' : c.cor, fontWeight: 700, fontSize: '.58rem', textTransform: 'uppercase', cursor: 'pointer', transition: 'all .18s', letterSpacing: .3, lineHeight: 1.2 }}
                          >{c.valor}</button>
                        ))}
                      </div>
                    </div>
              ...
              <div style={{ background: '#fafafa', borderTop: '1px dashed #E8E8E8', padding: '12px 16px' }}>
                <label style={{ fontSize: '.75rem', color: '#666', fontStyle: 'italic', fontWeight: 500, display: 'block', marginBottom: 5 }}>Observações (opcional) — registre situações específicas deste bloco:</label>
                <textarea
                  style={{ width: '100%', border: '1px solid #E8E8E8', borderRadius: 7, padding: '9px 11px', fontFamily: "'Inter',sans-serif", fontSize: '.82rem', color: '#2D2D2D', resize: 'vertical', minHeight: 60, outline: 'none' }}
        ...
        <button
          onClick={enviar}
          disabled={salvando}
          style={{ background: '#1A3C5E', color: '#fff', border: 'none', borderRadius: 10, padding: '15px 44px', fontFamily: "'Inter',sans-serif", fontSize: '.95rem', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 14px rgba(26,60,94,.25)', display: 'block', margin: '32px auto 0' }}
